...
Run `npm run check`, `npm run build`, `npm run verify:streamer-production-polish`, `npm run verify:live-card-snapshots`, `npm run verify:avatar-gateway`, and the avatar/snapshot storage tests. Start the local API/web plus Windows Docker PostgreSQL and Redis, run `npm run verify:staging`, then reset demo data with `npm run db:seed`.

Sign in as the synthetic streamer and remain offline. Confirm no manual room-cover field is present and navigation or metadata changes do not create media. For an explicitly authorized Cloudflare browser-live test, wait for authoritative LIVE plus five seconds, then confirm one immutable 640×360 snapshot appears on the live discovery card. Confirm no snapshot is accepted for local simulation, offline, unpublished, or another creator's room; a repeat inside five minutes returns a bounded retry response. End the broadcast and confirm discovery no longer returns the snapshot and offline recommendations use the creator avatar. Enable camera preview and public broadcasting only when device/live testing is explicitly intended.
...
Run `npm run verify:audience-ia-cleanup` with the local API, PostgreSQL, and Redis available. It verifies canonical account/navigation source contracts, Community retirement, zero mutation from read-only navigation endpoints, empty offline chat history, and server refusal of an audience realtime join to an offline room. Then test `/`, `/account/following`, `/account/activity`, `/account/notifications`, and `/account/preferences` at desktop and 390×844. Confirm Enter in the shared search field navigates to `/?q=...` from an account or room route. `/discover` is now a compatibility redirect to `/`.

## Radical audience-simplicity verification

Run `npm run verify:radical-audience-simplicity`, then `npm run db:migrate`, `npm run check`, `npm test`, `npm run build`, and the complete staging gate. Test `/` at 390×844, tablet, 1440px, and wide desktop. Confirm `/discover` and legacy `/tags` links replace to `/`, exact six-digit room IDs open directly, filter state survives history navigation, and offline rooms create no chat/presence connection or visit record. Migration `031_public_room_ids.sql` is required before starting the updated API.
