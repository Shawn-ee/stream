...
for (const rule of [
  ...
  /\.studio-open \.phase-preview \.broadcast-details-panel\s*\{[\s\S]*grid-row:\s*1 \/ span 3;[\s\S]*height:\s*100%;[\s\S]*overflow:\s*hidden/,
  /\.studio-open \.phase-preview \.broadcast-device-console\s*\{[\s\S]*grid-column:\s*1;[\s\S]*grid-row:\s*3/,
  /\.broadcast-device-console \.device-grid select\s*\{[\s\S]*min-width:\s*0;[\s\S]*max-width:\s*100%;[\s\S]*width:\s*100%/,
  ...
  /@media \(min-width: 1024px\)[\s\S]*\.studio-open \.broadcaster-page\s*\{[\s\S]*height:\s*calc\(100dvh - 4rem\);[\s\S]*overflow:\s*hidden/,
  /@media \(min-width: 1024px\)[\s\S]*\.studio-open \.broadcaster-stage \.quick-live-panel\.phase-preview,[\s\S]*grid-template-columns:\s*minmax\(0, 1\.55fr\) minmax\(22rem, 0\.95fr\)/,
  /\.studio-open \.broadcaster-page:is\(\.phase-live, \.phase-connecting, \.phase-ending\) \.broadcaster-layout\s*\{[\s\S]*height:\s*100%;[\s\S]*min-height:\s*0/,
