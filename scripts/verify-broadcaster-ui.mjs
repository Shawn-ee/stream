...
for (const rule of [
  ...
  /\.studio-open \.broadcaster-stage \.quick-live-panel\.phase-preview \.broadcast-details-panel\s*\{[\s\S]*grid-row:\s*1 \/ span 3;[\s\S]*position:\s*sticky/,
  /\.studio-open \.broadcaster-stage \.quick-live-panel\.phase-preview \.broadcast-device-console\s*\{[\s\S]*grid-column:\s*1;[\s\S]*grid-row:\s*3/,
  /\.studio-open \.broadcaster-stage \.quick-live-panel\.phase-preview \.broadcast-audience-preview\s*\{[\s\S]*display:\s*none/,
  ...
  /@media \(min-width: 1024px\)[\s\S]*\.studio-open \.broadcaster-stage \.quick-live-panel\.phase-preview,[\s\S]*grid-template-columns:\s*minmax\(0, 1\.35fr\) minmax\(20rem, 0\.85fr\)/,
