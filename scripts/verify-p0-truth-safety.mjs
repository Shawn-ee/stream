...
assert.match(app, /rooms\.filter\(\(item\) => \(item\.broadcast_state \?\? item\.status\) === "live" && item\.broadcast_status_source !== "local"\)/);
assert.match(app, /Creators to follow/);
...
assert.match(discovery, /SIMULATED STATUS/);
