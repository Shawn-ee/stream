...
try {
  ...
  const communityTags=await call("/api/discovery/tags?type=COMMUNITY",null);
  const music=publicTags.body.tags.find((item)=>item.slug==="music");
  const community=communityTags.body.tags.find((item)=>item.slug==="new-creators");
  assert.ok(music&&community);
  const updated=await call(`/api/streamer/rooms/${slug}`,streamer,{method:"PUT",body:{title:original.title,primaryLanguage:"en",additionalLanguages:["zh","ja"],tagIds:[music.id,community.id]}});
  ...
  assert.deepEqual(updated.body.room.tags.map((item)=>item.slug),["music","new-creators"]);
