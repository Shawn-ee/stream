...
- The immutable historical baseline is commit `8aa41bf688336c1f8a0a8478e69d556d094477b5` with annotated tag `stream-launch-candidate-0.1.0`. The current local application-source candidate is commit `4cadd6a631544377b826ea998dcd1102d5f5799c`, which adds environment, dependency-supply-chain, and guarded private-staging-operator hardening. It still requires publication approval and is not tagged; application-image digests must be recorded after an approved staging build so host rollback identifies both source and deployed images.
