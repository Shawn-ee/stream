...
- Pre-live metadata is saved before WHIP publication. Manual room-cover selection is removed. For browser broadcasts, the existing camera preview produces a single 640×360 WebP only after Cloudflare reports the room LIVE, with a ten-minute client refresh cadence and a five-minute server acceptance floor. The server revalidates room ownership and authoritative live state, normalizes again, strips metadata, audits replacement, and exposes the immutable asset only while the room remains genuinely live. Provider-side OBS capture is deferred; its future source is reserved in the schema.
...
- Pre-live title, language, and tags remain available before camera/microphone permission. Automatic audience-card snapshots begin only after explicit broadcast start and authoritative Cloudflare LIVE state.
